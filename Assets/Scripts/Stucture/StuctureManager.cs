...
 
        _ghostBuilding = Instantiate(curBuildingPrefab, cursorPos, Quaternion.identity);
        _ghostBuilding.GetComponent<FindBuildingSite>().plane.SetActive(true);
 
        buildingCursor = _ghostBuilding;
...
            return;
 
                    buildingCursor.GetComponent<FindBuildingSite>().plane.SetActive(false);
